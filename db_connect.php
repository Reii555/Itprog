...
if ($admins_count == 0) {
    ...
    $admin_accounts = mysqli_query($conn, "SELECT account_id FROM ACCOUNTS WHERE role='Admin' OR role='IT'");
    $admin_account_ids = [];
    while ($acc = mysqli_fetch_assoc($admin_accounts)) {
        $admin_account_ids[] = $acc['account_id'];
    }
    
    // If no admin accounts exist, create them
    if (empty($admin_account_ids)) {
        ...
        $admin_account_ids = [$admin_account_id, $it_account_id];
    }
    
    // Insert administrators
    $admin_data = [
        ['Super Admin', 'User', $admin_account_ids[0], 912345678],
        ['IT', 'Administrator', $admin_account_ids[1] ?? $admin_account_ids[0], 923456789]
    ];
...
if ($applications_count == 0) {
    ...
    if (!empty($student_ids) && !empty($scholarship_ids)) {
        $statuses = ['Draft', 'Submitted', 'Under Review', 'Approved', 'Rejected', 'Waitlisted'];
        ...
        if (isset($student_ids[1]) && isset($scholarship_ids[2])) {
            $insert4 = "INSERT INTO APPLICATIONS (student_id, scholarship_id, status, submission_date) 
                       VALUES ({$student_ids[1]}, {$scholarship_ids[2]}, 'Draft', NULL)";
        ...
        if (isset($student_ids[3]) && isset($scholarship_ids[2])) {
            $insert7 = "INSERT INTO APPLICATIONS (student_id, scholarship_id, status, submission_date) 
                       VALUES ({$student_ids[3]}, {$scholarship_ids[2]}, 'Submitted', '{$dates[2]}')";
...
if (!isset($_SESSION['student_id'])) {
    ...
    if (mysqli_num_rows($first_student) > 0) {
